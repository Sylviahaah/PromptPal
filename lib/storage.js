...
const Storage = {
    ...
    async savePrompt(prompt) {
        try {
            ...
            // Auto-detect variables if VariableUtils is available
            let detectedVariables = prompt.variables || [];
            let isStructured = false;

            if (typeof VariableUtils !== 'undefined') {
                detectedVariables = VariableUtils.extractVariablesForPrompt(
                    prompt.content,
                    prompt.variables || []
                );
                isStructured = detectedVariables.length > 0;
            }
    ...
    async updatePrompt(id, updates) {
        try {
            ...
            if (updates.content && typeof VariableUtils !== 'undefined') {
                const existingVars = updates.variables || prompts[index].variables || [];
                updates.variables = VariableUtils.extractVariablesForPrompt(
                    updates.content,
                    existingVars
                );
                updates.isStructured = updates.variables.length > 0;
            }
